...
extension WrapExtension on Logger {
  T wrap<T>(T Function() invocation, {int level = 0}) {
    ...
    finest('Invocation: $member');

    try {
      return invocation();
    } catch (e, stackTrace) {
      severe('Invocation threw: $member', e, stackTrace);
      rethrow;
    }
  }
  ...
  Future<T> wrapAsync<T>(Future<T> Function() invocation, {int level = 0}) async {
    ...
    try {
      ...
      finest('Invocation completed: $member, ${stopwatch.elapsedMilliseconds}ms');
    }
  }
}
...
class LoggerSignalsObserver extends SignalsObserver {
  ...
  void onSignalCreated<T>(Signal<T> instance, T value) {
    log.signals.finest('Created [${instance.globalId}, ${value.runtimeType}] SIGNAL');
  }

  @override
  void onSignalUpdated<T>(Signal<T> instance, T value) {
    log.signals.finest('Updated [${instance.globalId}, type ${value.runtimeType}] SIGNAL');
  }

  @override
  void onComputedCreated<T>(Computed<T> instance) {
    log.signals.finest('Created [${instance.globalId}] COMPUTED');
  }

  @override
  void onComputedUpdated<T>(Computed<T> instance, T value) {
    log.signals.finest('Updated [${instance.globalId}, ${value.runtimeType}] COMPUTED');
  }

  @override
  void onEffectCreated(Effect instance) {
    log.signals.finest('Created [${instance.globalId}] EFFECT');
  }

  @override
  void onEffectCalled(Effect instance) {
    log.signals.finest('Called [${instance.globalId}] EFFECT');
  }

  @override
  void onEffectRemoved(Effect instance) {
    log.signals.finest('Removed [${instance.globalId}] EFFECT');
  }
}
